...
def get_img_mask_hdf5(file_path, mask_size=256, augmentation_mode=False):
    ...
    write_hdf5(img_array_temp_hdf5, file_path + 'img.hdf5')
    write_hdf5(mask_array_temp_hdf5, file_path + 'mask.hdf5')
...
def get_img_mask_list(file_number, file_path, mask_size=256, augmentation_mode=False):
    ...
    img_ndarray = np.empty(shape=(len(img_file_list), mask_size, mask_size, 3), dtype=np.uint8)
    label_ndarray = np.empty(shape=(len(label_file_list), mask_size, mask_size, 1), dtype=np.uint8)

    num_file = 0
    for img, label in zip(img_list, label_list):
        img_ndarray[num_file, :, :, :] = img
        label_ndarray[num_file, :, :, 0] = label[:, :, 0]
        num_file += 1

    img_dataset = tf.data.Dataset.from_tensor_slices(img_ndarray)
    label_dataset = tf.data.Dataset.from_tensor_slices(label_ndarray)
